...
# this is a function that create the service with which we connect to the google calendar API to access user's resources and perform operations 
def create_service(user):
    ...
    # if the credentials is none or not valid, we try to use refresh token to get a new access token. If it is none, we request for the user authorize the application again. 
